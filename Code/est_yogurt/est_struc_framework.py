...
]] # creating a filtered DF with shopping trips, hh, week, purchase_date, store, flavor

yog    = master_df[(master_df['product_module_code'] == 3612) | (master_df['product_module_code'] == 3603)] # mask for yogurt
hh_yog = yog['household_code'].unique() # households that buy yogurt
...
) # making each decision a row
occ_lists.name = 'yogurt_buy' # naming the variable
hh_trips = yog_buyers.merge(
    occ_lists, on=['household_code', 'trip_code_uc'], how='left'
) # merging the yogurt decision on trips
trip_level = hh_trips.explode('yogurt_buy') # each item is a unique occasion
...
console.print(len(trip_level.columns))
console.print('household_income_z' in trip_level.columns)
console.print('weeks_since_last_flavor_z' in trip_level.columns)
console.print('since_last_trip_z' in trip_level.columns)
console.print('head_age_z' in trip_level.columns)
