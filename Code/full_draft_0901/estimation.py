...
flavors = pd.read_csv(
    "/scratch/dtm63837/Kilts_Panel/RMS/Reference_Documentation/2006-2020_Documentation/Latest_Flavor_2010.csv"
)  # load in flavors documentation
...
master_df["yogurt_buy"] = (
    master_df["product_module_code"].isin([3612, 3603])
    & (
        master_df["quantity"] > 0
    )  # create dummy for HH who bought at least one yogurt product
).astype(int)
