...
Results are shown in \ref{tab:res_tab}. Here, you can see mixed findings. Firstly, I find that $\gamma_i$ is statistically significant and negative at $-1.23$. This finding points to consumers being less willing to switch flavors between trips. This finding is surprising given the data fact that more than half of houses are switchers, conditional on yogurt purchase, and houses switch 3 times on average if they do switch. This discrepancy points to my sample potentially suffering from being too small. Further, I find a negative price coefficient $\alpha$, but am unable to claim statistical significance. This is fixable with a better instrumental variable, something which is actively being considered. Similarly, the control function correction coefficient is not significant either. However, given $\sigma$ is positive, we can infer that $\alpha$ is positively biased without this correction. With a better IV approach 

When looking at the characteristic coefficients, only berry, $\beta_i^b$, is statistically significant. This is, again, an issue with lack of observations. As \ref{tab:stat_tab} shows, berry has over 2,000 observed times being chosen. When looking at plain's coefficient, $\beta_i^p$, I do not find significance. This makes total sense upon seeing it is only chosen 412 times. The constant is negative, which is to be expected with a high outside option uptake rate. It is close to fringe significance, but is not either. 
