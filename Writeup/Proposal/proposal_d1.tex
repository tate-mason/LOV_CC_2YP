...
%============================================================%
    ...
    \subsection{CCP Approach}
      ...
      To solve the dynamic programming problem of the firm, I will use a Conditional Choice Probability (CCP) approach. This approach allows me to estimate the value function and the optimal advertising strategy without having to solve the full dynamic programming problem. Instead, I will estimate the CCPs directly from the data, and then use these estimates to recover the value function and the optimal advertising strategy. This approach is computationally efficient and can handle the high-dimensional state space that arises from the consumer's past consumption history and the firm's strategic advertising decisions. CCP estimation is useful in this context because it allows me to estimate the model with lower state-space dimensionality, which is important given the complexity of the model and the presence of unobserved heterogeneity across consumers.
